(ns ads_b_aviation.main
  "Kotodama WASM entrypoint for the ads_b_aviation clean-room actor (L4) — Clojure port.
  ...
  facts (`ads_b_aviation.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "ads_b_aviation")
(def tier "L4")
...
(defn page-limit
  ...
  src/ads_b_aviation/page_limit.kotoba."
  [requested]
  (if (pos? requested)
...
(defn emit-facts
  "EAVT facts for one record: {\"ads_b_aviation.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "ads_b_aviation-compat" :tier tier :entities entities} 200])
...
(defn start! [] :ads_b_aviation-compat/ready)
